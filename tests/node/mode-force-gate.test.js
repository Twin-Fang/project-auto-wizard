...
test("run(): TTY 환경에서 --force 없이 version 모드를 실행하면 즉시 거부되고 아무 파일도 쓰지 않는다", async () => {
  const target = mkdtempSync(join(tmpdir(), "paw-tty-version-"));
  try {
    const code = await withStubbedTTY(true, () => run(["--mode", "version", "--type", "node"], { cwd: target }));
    assert.strictEqual(code, 1);
    assert.ok(!existsSync(join(target, "version.yml")), "TTY라도 --force 없이는 파일을 쓰면 안 됩니다");
  } finally {
    rmSync(target, { recursive: true, force: true });
...
test("run(): TTY 환경에서 --force 없이 workflows 모드를 실행하면 즉시 거부되고 아무 파일도 쓰지 않는다", async () => {
  const target = mkdtempSync(join(tmpdir(), "paw-tty-workflows-"));
  try {
    const code = await withStubbedTTY(true, () => run(["--mode", "workflows", "--type", "node"], { cwd: target }));
    assert.strictEqual(code, 1);
    assert.ok(!existsSync(join(target, ".github")), "TTY라도 --force 없이는 파일을 쓰면 안 됩니다");
  } finally {
    rmSync(target, { recursive: true, force: true });
  }
});
